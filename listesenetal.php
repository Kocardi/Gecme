...
$reponse = $bdd->query('SELECT * FROM fichedevie f1 WHERE naturedinter="Départ en étalonnage"AND NOT EXISTS ( SELECT * FROM fichedevie f2 WHERE f1.ident = f2.ident AND f2.naturedinter ="Retour d\'étalonnage") AND NOT EXISTS (SELECT * FROM fichedevie f3 WHERE f1.ident = f3.ident AND f3.naturedinter ="Déclassement") ORDER BY datepicker3');
$reponse2 = $bdd->query('
SELECT * 
FROM liste 
